...
在一体化模型的评测中，如何**高效地同时评测多个模型与多个 benchmark** 一直是一个难点。传统做法往往需要为每个模型与每个 benchmark 单独编写脚本，流程分散、维护成本高。

为了解决这一问题，我们开发了 **UMMEvalKit** —— 一个通用的一体化模型评测工具包。它提供统一的评测入口与配置方式，支持多模型、多 benchmark 的并行评测，并提供简单易扩展的接口，方便你添加新的模型和 benchmark。同时，此工具包也可用于评测纯图像生成与图像编辑模型。
...
1. **多模型 × 多 benchmark 统一评测**  
   - 相比传统脚本通常只能评测「单模型 × 单 benchmark」的组合，UMMEvalKit 支持在一次运行中同时评测多个模型在多个 benchmark 上的表现；  
...
| `HF_HOME` | ❌ 不支持 | HuggingFace 总缓存目录，评测相关模型会缓存到此处。 | `HF_HOME="/path/to/UMMEvalKit/.cache/huggingface"` |
...
本节介绍如何在 UMMEvalKit 中**接入新模型**与**接入新 benchmark**。
...
> 重要：  
> - 在**训练模型**时，工作目录通常为仓库根目录（例如 `UMMEvalKit`）；  
